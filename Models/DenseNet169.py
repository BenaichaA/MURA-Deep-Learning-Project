...
from tensorflow.keras.layers import Input, ZeroPadding2D, Dense, Dropout, Activation, Conv2D, AveragePooling2D, GlobalAveragePooling2D, MaxPooling2D, BatchNormalization, Concatenate, Layer, InputSpec
...
def densenet169_CAM_model(img_rows, img_cols, color_type=1, nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.5, dropout_rate=0.0, weight_decay=1e-4, num_classes=None):
    eps = 1.1e-5
    ...
    x_newfc = GlobalAveragePooling2D(name='pool'+str(final_stage))(x)
    FCL = Dense(num_classes, name='fc6')
    x_newfc = FCL(x_newfc)
    x_newfc = Activation('sigmoid', name='prob')(x_newfc)

    model = Model(img_input, x_newfc)
    last_convolution = Model(img_input, x)
    
    opt1 = Adam()
    opt2 = Adam()

    model.compile(optimizer=opt1, loss='binary_crossentropy', metrics=['accuracy'])
    last_convolution.compile(optimizer=opt2, loss='binary_crossentropy', metrics=['accuracy'])

    return model, FCL, last_convolution
